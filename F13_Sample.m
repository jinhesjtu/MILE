%%  Performance versus path-loss exponent value
clear
close all
TrialAll = 500;
count = 0;
for PL = [1:0.1:4];
    disp(sprintf('PL: %.1f', PL));
    SnrdB = 20;
    count = count + 1;
    for trial = 1:TrialAll;
        ...
        for num = 1:NfAll
            ...
            A(:,num) = (Rng(num)./rngall).^(PL/2) .*exp(j *2*pi * (Rng(num) - rngall));
        end
...
for    PL = 1:0.1:4
    ...
    G = diag(randn(1,Sensor)*0.122 + 1);
    A = (r./rall).^(PL/2).*exp(-j*2*pi/Lambda * (rall - r));    
    for num = 1:SigAll
...
xlabel('Path loss exponent value'); ylabel('RMSE of angle estimates, in degrees')
title('(a): Performance of angle estimates')

axis([1,4,0.001,20])
figure; set(gcf,'DefaultLineLineWidth',1.5)
semilogy(D,Errrpha(1,:), '-o',  D,Crbr(1,:), '-k'); grid on; 
xlabel('Path loss exponent value'); ylabel('RMSE of range r_1 estimates, in wavelength')
