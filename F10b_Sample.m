%% Identification results for L = 7, K = 6;
clear
close all
TrialAll = 500;
for trial = 1:TrialAll;
    Sensor = 7;
    Snap = 1000;
    Doa = [-15 10 20 30 40 20];
    Rng = [  3  3   6  2000 3000 4000 ];
    SigAll = length(Doa); NfAll = length(Rng);
    Lambda = 1;  D = 0.5*Lambda;
    Sp = [-2  -1.1 -0.5 0 0.5 1.2  2];
    for num = 1:NfAll
    ...
    f = [0.2, 0.4, -0.12, -0.31, 0.43, 0.15, 0.39];
    for num = 1:SigAll
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Smd = (Sensor + 1)/2; ssa = Smd - 1; ssb = Smd+ 1; ssc = Smd - 3; ssd = Smd + 3;
...
semilogy(ufin(:,2), rfin(:,2), '*', ufin(:,3), rfin(:,3), '*', ufin(:,5), 2000*ones(TrialAll,1), '*', ufin(:,6), 3000*ones(TrialAll,1),'*',...
    ufin(:,1), rfin(:,1), '*', ufin(:,4), 4000*ones(TrialAll,1), '*'); grid on
axis([-20,45,1,10000])
title('(b)')
xlabel('Angle, in degree')
ylabel('Range, in wavelength')
legend('Source 1', 'Source 2', 'Source 3', 'Source 4', 'Source 5', 'Source 6', 'Location', 'NorthWest')
