%% Classfication and Localization 2-NF 2-FF underestimate K, K = 3
clear
close all
TrialAll = 500;
...
semilogy(N,rini(:,1),'.', N,rini(:,2), '.', N,rini(:,3), '.', N,DR*ones(1,TrialAll),'-k'); grid on;
axis([1,TrialAll, 1,1e5])
xlabel('Number of trials')
ylabel('Range, in wavelength')
legend('NF source 2', 'FF source 1', 'FF source 2', 'Boundary R')
...
N = 1:TrialAll;
figure; set(gcf,'DefaultLineLineWidth',1.5)
plot(N,ufin(:,1), '.', N,ufin(:,2), '.', N,ufin(:,3), '.'); grid on;
xlabel('Number of trials')
ylabel('Angle, in degrees')
legend('NF source 2', 'FF source 1', 'FF source 2')
