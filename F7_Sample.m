...
for SnrdB = 0:5:40
    ...
    for trial = 1:TrialAll;
        ...
        for num = 1:SigAll
            ...
            rstep = 0.001; ustep = 0.001;
            if rini(num) < 10;
                nr = 0;
                for dr = rini(num) - 0.3 :rstep: rini(num) + 0.3
                    nr = nr + 1; nu = 0;
                    for du = uini(num) - 0.3 : ustep: uini(num) + 0.3
                        nu = nu + 1;
                ...
                [b1 a1] = max(max(Pr)); rmu(num) = (a1 -1)*rstep + rini(num) - 0.3;
                [b1 a1] = max(max(Pr.')); umu(num) = (a1 -1)*ustep + uini(num) - 0.3;
            else
                dr = 5000; nu = 0;
                for du = uini(num) - 0.3 : ustep: uini(num) + 0.3
                    nu = nu + 1;
...
semilogy(D,Errupha(1,:), '-x', D,Errurefi(1,:), '-*', D,Crbu, '-k'); grid on;  
% axis([50 10000 1e-3 1])
xlabel('SNR, in dB'); ylabel('RMSE of angle \theta_1 estimates, in degrees')

figure; set(gcf,'DefaultLineLineWidth',1.5)
semilogy(D,Errrpha(1,:), '-*',D,Errrfefi(1,:), '-x', D,Crbr, '-k'); grid on;   
% axis([50 10000 1e-3 1])
xlabel('SNR, in dB'); ylabel('RMSE of angle r_1 estimates, in wavelength')
